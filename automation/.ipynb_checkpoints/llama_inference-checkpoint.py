...
def _load_prompts_payload(prompts_payload: dict | str | Path) -> dict:
    """Accept an in-memory payload, a prompts.json path, or a dir containing one."""
    if isinstance(prompts_payload, dict):
        return prompts_payload

    path = Path(prompts_payload)
    if path.is_dir():
        path = path / "prompts.json"
    if not path.exists():
        raise FileNotFoundError(f"no prompts.json at {path}")

    payload = json.loads(path.read_text(encoding="utf-8"))
    if not payload.get("prompts"):
        raise ValueError(f"{path} has no prompts")
    print(f"Loaded {len(payload['prompts'])} prompt pair(s) <- {path}")
    return payload
...
def run_inference(prompts_payload: dict | str | Path, df: pd.DataFrame, question_col: str,
    ...
    `prompts_payload` is either the dict `build_prompts` returned, a path to a
    prompts.json, or a directory holding one.
    """
    prompts_payload = _load_prompts_payload(prompts_payload)
...
def llama_inference(questions_csv: Path, output_dir: Path,
    ...
    writes prompts.json next to the output — skipped when `output_dir/prompts.json`
    already exists and `reuse_prompts` is True, so a rerun goes straight to
    answering. Phase 2 starts llama-server and answers every prompt
    concurrently. Every input column is preserved in the output response.csv; a
    `response` column is appended, holding either the model's answer or, if
    every retry was exhausted, an "ERROR: ..." string — no question is ever
    silently skipped.
    """
    ...
    prompts_path = output_dir / "prompts.json"
    if reuse_prompts and prompts_path.exists():
        prompts_payload = _load_prompts_payload(prompts_path)
    else:
